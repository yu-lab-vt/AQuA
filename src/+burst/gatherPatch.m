function [lblMap2F,resCellG] = gatherPatch(lblMap,lmAll,lmLocR,resCell)
% gatherPatch gather 3D super pixels from seed growing
...
for ii=1:numel(pixLstCutG)
    ...
    if pixcc.NumObjects>1
        ...
        %         iyySel = iyy*0;
        %         for ee=1:numel(iyy)
        %             if lmLocR(iyy(ee))==lmIdxG(ii)
        %                 iyySel(ee) = 1;
        %             end
        %         end
        %         idx2 = iyy(iyySel>0);
        %idx2 = find(lm1==lmIdxG(ii));  % find location for patch seed       
...
for ii=1:numel(pixLstCutG1)
    lblMap2(pixLstCutG1{ii}) = uint32(ii);
end
