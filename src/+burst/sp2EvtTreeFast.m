function [lblMapC,evtLst,bins] = sp2EvtTreeFast(lblMapS,riseX,propUR,propCT,exclRat)
...
%         keyboard
    ...
    if ~isinf(d0)
        % find path
        d2 = distances(G,t0(ix));
        idxSel = d2<=d2(s0);
        spPath = setdiff(sp(idxSel),sp0);        
        %P = shortestpath(G,s0,t0(1));
        %spPath = setdiff(sp(P),sp0);
        bins(spPath) = bins(sp0(ix));
